public class Date {
    ...
    public Date(String date) { // String constructor
        ...
        try {
            ...
            if (regex.equals(" ")) {
                month = Month.valueOf(dates[0].toUpperCase()).ordinal() + 1; // Converts the month (e.g., "AUG") to its numeric representation
                day = Integer.parseInt(dates[1].replace(",", "")); // Remove comma
            } else { // Regex is "/"; Parse as usual
                day = Integer.parseInt(dates[0]);
                month = Integer.parseInt(dates[1]);
            }
            year = Integer.parseInt(dates[2]); // Year is parsed the same for both cases
        } catch (IllegalArgumentException e) {
            System.out.println("Invalid input. (Use format 16/07/2025 OR JUL 16, 2025)");
        }
    }
